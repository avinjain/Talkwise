...
export default function ProfilePage() {
  ...
  const personalitySection = (
    <section>
        ...
        <h2 className="text-base font-semibold text-slate-900">Discover your personality</h2>
      </div>

      {!hasResult || !scores ? (
        <div
          onClick={() => router.push('/profile/test')}
          className="group cursor-pointer bg-white rounded-2xl border-2 border-dashed border-slate-200 hover:border-brand-300 hover:bg-brand-50/30 p-6 text-center transition-all duration-200"
        >
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-brand-500 to-accent-500 flex items-center justify-center mx-auto mb-3 shadow-md shadow-brand-500/20 group-hover:scale-105 transition-transform">
            <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3" />
            </svg>
          </div>
          <p className="text-slate-600 font-medium mb-1 text-sm">Discover your communication personality</p>
          <p className="text-xs text-slate-500 mb-4">27 questions &middot; 9 traits &middot; AI feedback</p>
